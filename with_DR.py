...
# Dry-run (peut être forcé par --dry-run)
DRY_RUN = False
...
# -----------------------------
def vc_connect():
    # pyVmomi 8.0U3: compatibilité Python 3.12, SmartConnect accepte sslContext
...
def sonic_rest_post(path, payload):
    if DRY_RUN:
        print(f"[DRY-RUN] Would POST {path} payload={payload}")
        return None
    url = f"https://{SONIC_HOST}/restconf/data/{path}"
...
def sonic_rest_patch(path, payload):
    if DRY_RUN:
        print(f"[DRY-RUN] Would PATCH {path} payload={payload}")
        return None
    url = f"https://{SONIC_HOST}/restconf/data/{path}"
...
def ensure_vlan_interface(vlan_id: int):
    ...
    sonic_rest_post("openconfig-interfaces:interfaces", payload)
    ...
    try:
        ...
        def update_port(iface: str):
            ...
            if set(final) != current_trunk:
                ...
                if DRY_RUN:
                    print(f"[DRY-RUN] Would set cache[{iface}] = {sorted(list(managed_desired))}")
                else:
    ...
    try:
        ...
        for port, desired_managed in desired_managed_per_port.items():
            ...
            if set(final) != current_trunk:
                ...
                if DRY_RUN:
                    print(f"[DRY-RUN] Would set cache[{port}] = {sorted(list(desired_managed))}")
                else:
        ...
        for port in list(_port_vlan_cache.keys()):
            if port not in desired_managed_per_port:
                ...
                if set(final) != current_trunk:
                    set_trunk_vlans(port, final)
                    print(f"[RECONCILE{' DRY-RUN' if DRY_RUN else ''}] Port {port}: décommission géré, trunk-vlans -> {final}")
                if not DRY_RUN:
        ...
        for vid in candidates:
            if not vlan_present_on_any_port(vid):
                if DRY_RUN:
                    print(f"[DRY-RUN] Would DELETE Vlan{vid} (no longer used)")
                else:
...
def main_daemon():
    global DRY_RUN
    parser = argparse.ArgumentParser(description="VM→SONiC VLAN daemon")
    parser.add_argument("--dry-run", action="store_true", help="simulate all SONiC write operations")
    ...
    try:
        ...
        last_key = None
        last_reconcile = 0
        print(f"[Daemon] started (dry-run={DRY_RUN}); watching VM network events + periodic reconcile...")
